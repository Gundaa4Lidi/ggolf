...
crop.service('cropEXIF', ['$log', function ($log) {
  ...
  function getImageData(img, callback) {
    ...
    if (img.src) {
      ...
      var fileReader = new FileReader();
      fileReader.onload = function (e) {
        $log.debug('getImageData: Got file of length %o', e.target.result.byteLength);
        handleBinaryFile(e.target.result);
      };
  ...
  function readTags(file, tiffStart, dirStart, strings, bigEnd) {
    ...
    for (i = 0; i < entries; i++) {
      ...
      if (tag) {
        ...
        $log.warn('Unknown tag: ' + file.getUint16(entryOffset, !bigEnd));
      }
    }
    return tags;
  }
  ...
  function readEXIFData(file, start) {
    if (getStringFromDB(file, start, 4) != "Exif") {
      $log.error("Not valid EXIF data! " + getStringFromDB(file, start, 4));
      return false;
    }
    ...
    if (file.getUint16(tiffOffset) == 0x4949) {
      ...
      $log.error("Not valid TIFF data! (no 0x4949 or 0x4D4D)");
      return false;
    }

    if (file.getUint16(tiffOffset + 2, !bigEnd) != 0x002A) {
      $log.error("Not valid TIFF data! (no 0x002A)");
      return false;
    }
    ...
    if (firstIFDOffset < 0x00000008) {
      $log.error("Not valid TIFF data! (First offset less than 8)", file.getUint32(tiffOffset + 4, !bigEnd));
      return false;
    }
